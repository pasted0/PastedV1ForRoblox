print("went to bedwars | Check To See If Dlta actually looks at the thingy " .. math.random(100))
...
-- patched -- 
local MelodyExploit = vape.Categories.Utility:CreateModule({
    Name = "MelodyExploit",
    Function = function(callback)
      while callback do
        task.wait()
        local args = {
          [1] = {
            ["healTarget"] = game:GetService("Players").LocalPlayer.Character
          }
        }
        game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.PlayGuitar:FireServer(unpack(args))
        task.wait(1)
        game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.StopPlayingGuitar:FireServer()
      end
    end,
    Tooltip = 'Waits until your health goes down then constanly fires the melody remotes putting your name as the one to heal'
})

local nightmare = vape.Categories.Utility:CreateModule({
    Name = "NightmateEmote",
    Function = function(callback)
...
local HitSelect = vape.Catagories.Combat:CreateModule({
  ...
  Function = function(callback)
    for _,v in playersService:GetPlayers() do
      local target = v
      local targetHumanoid = target.Character:WaitForChild("HumanoidRootPart")
      local targetRoot = target.Character:WaitForChild("HumanoidRootPart")
      local dist = (root.Position - targetRoot.Position).Magnitude
      
      if dist >= 10 and callback then
        local function isHoldingTool()
          for _,v in target.Character:GetChildren() do
            local suc,err = pcall(function()
              v:FindFirstChildWhichIsA("Tool")
            end)
            return suc
          end
          local holdingTool = isHoldingTool()
          if holdingTool then
            local targetTool = target.Character:FindFirstChildWhichIsA("Tool")
            targetTool:GetPropertyChangedSignal("Active"):Connect(function()
              if targetTool.Activated == true then
              end
            end)
          end
        end
    ...
    Function = function(callback)
        ...
        if callback then
            humanoid.HealthChanged:Connect(function()
                if humanoid.Health <= trigger and callback then
                    indanger = true
                    root.CFrame = root.CFrame + Vector3.new(0,2500,0)
                    else
                        indanger = false
                        root.CFrame = root.CFrame - Vector3.new(0,2450)
                end
            end)
...
local InfiniteJump = vape.Categories.Blatant:CreateModule({
    Name = "InfiniteJump",
    Function = function(callback)
